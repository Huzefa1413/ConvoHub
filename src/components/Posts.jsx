        ...
        <div className="post">
            ...
            {(props.post.userId === auth.currentUser.uid) ?
                <div className="buttonbox">
                    <button onClick={() => { deletePost(props.post.id) }}><img src={bin} alt='delete' />Delete</button>
                    <button style={(edit) ? { display: 'none' } : { display: 'flex' }} onClick={() => { setEdit(true); setEditText(props.post.text) }}><img src={edits} alt='edit' />Edit</button>
                    <button style={(edit) ? { display: 'flex' } : { display: 'none' }} onClick={() => { setEdit(false); props.post.text = editText; updatePost(props.post.id, props.post.text) }}><img src={updateicon} alt='update' />Update</button>
                </div> :
                <div className="buttonbox">
